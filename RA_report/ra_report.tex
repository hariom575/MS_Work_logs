    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.50\linewidth,height=0.50\texthei\newcommand{\rupee}{\textrupee}ght, keepaspectratio]{notification-permission.png}
        \hspace{0,3cm}
...
\begin{enumerate}
    ...
    \item Removed unused images from the server as space was getting fulled.
    \item Added shared docker network used by both Nginx and gaurav's web application.
    \item Increased version code and version No. for app update on play store.
      \item merged all my changes into single branch before creating the .aab file from both RA and MS work.
      
    \item Coordinated with sir and sent him updated .aab file and screenshots of new UI.
\end{enumerate}
\section{Apr 06 2026 to Apr 09 2026}
{\bf No. of Hours: 8}
\subsection{Work Done}
\begin{enumerate}
    \item Studied firebase crash analytics tool.
    \item Added firebase crash Analytics in the app.
    \item Created firebase account with my personal gmail.
    \item Exported service.json file inside the app code directory.
    \item Added firebase dependencies and   plugins inside the app.
    \item Added firebase service inside the app code.
    \item Created a test crash in Launcher main file.
    \item Verified the analytics dashboard inside Android studio.
\end{enumerate}
\subsection{ScreenShot from Firebase Crash Analytics Dashboard}

  \begin{figure}[H]
        \centering
        \includegraphics[width=1.0\linewidth,height=0.50\textheight, keepaspectratio]{firebase.png}
        
        \caption{Firebase crash analytics dashboard}
        \label{fig:firebase_crash_analytics}
    \end{figure}
\section{May 25 2026 to May 25 2026}
{\bf No of Hours: 1}
\subsection{To Do}
\begin{enumerate}
    \item Add the new functionality of sending notification at every day 5 pm to all the participants requesting them to fill daily survey.
    \item Create API at the server side to accept this data and store it in db.
    \item Based on pilot study feedback:
    \begin{enumerate}
        \item Make the LOCK screen optional
        \item Don't ask the lock screen and Read phone state data permission during onboarding
        \item Instead from the setting button present at the home screen give the option to make JoDo as lock screen and while making JoDo as lock screen ask permission to read call state (This permission is required to destroy lock screen overlay dynamically when there is call is call and lock screen overlay is covering the lock screen)
        \item Remove the notification foreground service and use android accessibility service for foreground background detection as longer running service are forbidden in new devices also it consumes too much battery. 
    \end{enumerate}
\end{enumerate}
\end{document}
